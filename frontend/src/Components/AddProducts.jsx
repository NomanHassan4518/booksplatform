...
const AddProducts = () => {
    ...
    const submit = async (e) => {
        e.preventDefault()
        let result = await fetch("https://booksplatform-theta.vercel.app/addproduct", {
            method: "POST",
            body: JSON.stringify({ name, img,price,category,subcategory,desc,stock }),
            headers: {
    ...
    return (
        <div className='w-full flex items-center justify-center py-20'>
            <div className='border w-[60%] p-14 space-y-4'>
                <input type="text" className='border rounded p-3 focus:outline-none block w-full ' placeholder='Enter Image Name' value={name} onChange={(e)=>{setName(e.target.value)}} />
                <input type="text" className='border rounded p-3 focus:outline-none block w-full ' placeholder='Enter Image URL' value={img} onChange={(e)=>{setImg(e.target.value)}} />
                <input type="text" className='border rounded p-3 focus:outline-none block w-full ' placeholder='Enter Image Price' value={price} onChange={(e)=>{setPrice(e.target.value)}} />
